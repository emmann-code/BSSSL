...
class _StaffAdvanceRequestPageState extends State<StaffAdvanceRequestPage> {
  ...
  void initState() {
    super.initState();
    fetchReferenceNumber(); // Fetch reference number on initialization
  }

  Future<void> fetchReferenceNumber() async {
    try {
      final referenceData = await getReferenceNumber();
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Failed to fetch reference number')),
      );
    }
  }

  Future<void> saveAdvanceRequest({required bool asDraft}) async {
    try {
      // Collect data
      final recordData = {
        ...
        "isDraft": asDraft, // Pass a flag for draft or final submission
      };
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(asDraft
              ...
              : 'Request submitted for processing successfully'),
        ),
      );
    } catch (e) {
      print('Error saving advance request: $e');
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Failed to save advance request')),
      );
    }
  }
            ...
            child: Column(
              ...
              children: [
                ...
                CustomTextField(
                  ...
                  controller: referenceController,
                  maxLines: 1,
                  // readOnly: true, // Reference number is fetched and not editable
                  inputFormatters: [],
                ),
                const SizedBox(height: 16),
                CustomLabel(text: "Type of Advance"),
                ...
                Row(
                  ...
                  children: [
                    CustomButton(
                      text: "Save as Draft",
                      onPressed: () => saveAdvanceRequest(asDraft: true),
                      color: Theme.of(context)
                          .colorScheme
                          .background
                          .withOpacity(0.009),
                    ...
                    CustomButton(
                      text: "Send for further \n  processing",
                      onPressed: () => saveAdvanceRequest(asDraft: false),
                      color: Theme.of(context).colorScheme.secondary,
                    ),
                  ],
                ),
